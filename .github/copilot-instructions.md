...
- Treat performance as a first-class engineering principle. Prefer designs that minimize allocations in low-level reader/writer paths, and justify more complex optimizations with targeted measurements.
- Keep low-level serialization APIs allocation-conscious and suitable for NativeAOT and trimming scenarios. Avoid hidden serializer-engine dependencies and mutable global state.
- CPU intrinsics may be used for hot paths when measurement justifies them, but prefer a simple correct implementation before adding hardware-specific optimizations.
...
- When adding low-level serialization code, prefer spans, pooled buffers, and streaming-friendly APIs over string- or array-heavy implementations unless a measured tradeoff justifies otherwise.
